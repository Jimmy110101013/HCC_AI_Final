...
class Task1:
    def __init__(self, drone, x_target, y_target, z_target):
        self.drone = drone
...
class Task2:
    def __init__(self, drone, detector_url, x_target, y_target, z_target, tolerance_x=50, tolerance_y=50, tolerance_z=0.1):
        self.drone = drone
    ...
    def send_frame_for_detection(self, frame):
        ...
        response = requests.post(self.detector_url, data=img_encoded.tostring(), headers={'Content-Type': 'application/octet-stream'})
        return response.json()  # Assuming the response returns a JSON with detection results
    ...
    def run(self, frame, tag_list):
        # Send frame to remote server for detection
        detection_results = self.send_frame_for_detection(frame)

        # Determine target AprilTag based on detection results
        if detection_results:
            detection_class = detection_results['class']
            self.determine_target_tag(detection_class)
